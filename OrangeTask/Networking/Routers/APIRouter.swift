...
//        urlRequest.setValue(UIDevice.current.identifierForVendor?.uuidString ?? String(), forHTTPHeaderField: HTTPHeaderField.macAddress.rawValue)
    ...
    func send<T: Codable>(data: [UploadData]? = nil, completion: @escaping (_ response: T) -> Void) {
        ...
        if let data = data {
            uploadToServerWith(data: data) { (respons: T?, errorType) in
