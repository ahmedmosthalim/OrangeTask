...
class MGFloatingTextField: UITextField {
    ...
    func handelPlaceholderView() {
        ...
        let width = pHolder.width(withConstrainedHeight: 16, font: UIFont(name: FontFamilySize.medium.size, size: 15)!) + 30
    ...
    func createLabelView() {
        ...
        guard let pHolder = self.placeholder else {return}
        let width = pHolder.width(withConstrainedHeight: 16, font: UIFont(name: FontFamilySize.regular.size, size: 15)!)
        let label = UILabel(frame: CGRect(x: 15, y: 0, width: width, height: 16))
        label.text = self.placeholder
        label.textColor = self.selectedBorderColor
        label.font = UIFont(name: FontFamilySize.regular.size, size: 15)
    ...
    @objc func fireTimer() {
        self.layer.borderColor = (self.layer.borderColor! == self.normalBorderColor.cgColor) ? self.selectedBorderColor.cgColor : self.normalBorderColor.cgColor
    }
}
