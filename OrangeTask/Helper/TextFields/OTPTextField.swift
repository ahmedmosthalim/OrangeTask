...
class OTPTextField: UITextField, UITextFieldDelegate {
    ...
    private func createLabelsStackView(with count: Int) -> UIStackView {
        ...
        for _ in 1 ... count {
            ...
            label.layer.borderColor = Theme.colors.borderColor.cgColor
            label.clipsToBounds = true
            label.textColor = Theme.colors.mainDarkFontColor
            label.backgroundColor = Theme.colors.textFielBgColor
            ...
            bottomLine.backgroundColor = Theme.colors.borderColor.cgColor
    ...
    @objc private func textDidChange() {
        guard let text = text, text.count <= digitalLabels.count else { return }
        for i in 0 ..< digitalLabels.count {
            let currentLabel = digitalLabels[i]
            if i < text.count {
                let index = text.index(text.startIndex, offsetBy: i)
                currentLabel.text = String(text[index])
                digitalLabels[i].layer.borderColor = Theme.colors.mainColor.cgColor
            } else {
                currentLabel.text = defaultText
                digitalLabels[i].layer.borderColor = Theme.colors.borderColor.cgColor
            }
        }
